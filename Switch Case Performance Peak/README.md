...
It is here that we finally get to _fully and truly_ optimize the `switch case` statement so that the developer is confident of using such optimizations in their codebase without the excruciating pain of complexity, and nightmare maintenance problems and concerns each time there is, all but, a small addition to the codebase of code logic.

The main focus of the [S2S Compilers: Understanding Switch Case Statements](https://dev.to/alex01/s2s-compilers-understanding-switch-case-statements-4ibk) blog is to get the readers up to speed of what the switch case is and to contextualize various implementions that the developer can _attempt_ in order to reach the performance peak.
...
Contrary to the statement above - after doing some research, I have finally managed to put together a jump table generated by the `switch case` statement that truly uses the smallest number of assembly instructions overall without any necessity of crafting any assembly code from the group up.
