...
class CustomMemoryStorage {
  ...
  CustomMemoryStorage()
  {
    ...
    // NOTE: buffer = (unsigned char*)alloca(Size); // This is NOT POSSIBLE DUE TO THE BROKEN ALLOCA RAII FOR C++. THIS MEMORY IS RESTORED AS SOON AS THE FUNCTION HAS RETURNED. THE DESTRUCTOR CAN'T TAKE CARE OF THIS. THE ONLY WAY TO HAVE ALLOCA WORK IS TO ATTACH THE BUFFER VIA "void attach_buffer(void*)" from the main scope GIVEN THAT THE ALLOCA BUFFER IS RESTORED AT THE END OF THE CURRECT FUNCTION SCOPE. CHECK OUT THE ALLOCA IN THE MAIN BELOW.
