#include <memory>

#include <iostream>

#include <cassert>


#define start_time clock_t s_t_a_r_t = clock();

#if __GNUG__
#define end_time printf("[Cpu_time_used: %f]\n", static_cast<double>(clock() - s_t_a_r_t) / CLOCKS_PER_SEC);
#else
#define end_time printf("[Cpu_time_used: %f]\n", ((double) (clock() - s_t_a_r_t)) / CLOCKS_PER_SEC);
#endif

using namespace std;

struct TreeNode {

    int val { 0 };

    shared_ptr<TreeNode> left { nullptr };
    shared_ptr<TreeNode> right { nullptr };

    TreeNode() {}

    TreeNode(int val) : val{val} {}

};


struct CallFrame {

    // args
    struct Args {
        const shared_ptr<TreeNode>* node { nullptr };
    } args;


    // Return Vals
    struct Ret {
        void* origin { nullptr };
        int value { 0 }; // RETURN VALUE
        // int value { -1000 }; // NOTE: SHOULD WORK TOO
    } ret;

    CallFrame() {}


    CallFrame(const CallFrame& other) {
        args.node = other.args.node;
        ret.origin = other.ret.origin;
        ret.value = other.ret.value;
    }


    CallFrame(CallFrame&& other) {
        args.node = other.args.node;
        ret.origin = other.ret.origin;
        ret.value = other.ret.value;
    }

    CallFrame& operator=(const CallFrame& other) {

        args.node = other.args.node;
        ret.origin = other.ret.origin;
        ret.value = other.ret.value;

        return *this;
    }

    CallFrame& operator=(CallFrame&& other) {
        args.node = other.args.node;
        ret.origin = other.ret.origin;
        ret.value = other.ret.value;

        return *this;
    }

    ~CallFrame() {}

};

template<class T, size_t Size>
class StaticStorage {
  public:
  using ItemType = T;
  unsigned char buffer[sizeof(T) * Size];
  size_t stack_count = 0;

  StaticStorage() {}


  // NOTE: Deleted for now. Will think about this
  StaticStorage(const StaticStorage&) = delete;
  StaticStorage(StaticStorage&&) = delete;

  StaticStorage& operator=(const StaticStorage&) = delete;
  StaticStorage& operator=(StaticStorage&&) = delete;

  void add(const T& item) {
    new(&buffer[stack_count * sizeof(T)]) T(item);
    stack_count++;
  }

  void add(T&& item) {
    new(&buffer[stack_count * sizeof(T)]) T(std::move(item));
    stack_count++;
  }


  // T* pop();
  // NOTE: POP FOR STATIC STORAGE IS ILL_ADVISED, RESULTING IN MEMORY LEAKS (EVEN WHEN THERE IS A CAST, WE ARE STILL POINTINT TO UCHAR BUFFER)
  // MAKE SURE TO TEST THIS AT the -O0 level optimizations CUZ THE COMPILER JUST IS TAKES STUFF OUT AT O3
  T& pop() { // SAME GOES FOR const T&
    stack_count--;
    T& popped = *reinterpret_cast<T*>(buffer + ( stack_count * sizeof(T)));

    // *reinterpret_cast<T*>(buffer + ( stack_count * sizeof(T))) = T();


    return popped;
  }

  T& at(size_t i) {
    return *reinterpret_cast<T*>(buffer + ( i * sizeof(T)));
  }

  T& last() {
    return *reinterpret_cast<T*>(buffer + ( (stack_count-1) * sizeof(T)));
  }

  // TODO: WRAP UP IN A MACRO JUST LIKE _storage_add
  void insert(size_t i, T&& item) {
    T* _at = reinterpret_cast<T*>(buffer + ( i * sizeof(T)));
    *_at = std::move(item);
    // ALTERNATIVELY
    // _at->~T();
    // new(&buffer[i * sizeof(T)]) T(std::move(item));
  }


  ~StaticStorage() {
    for(size_t i = 0; i < stack_count; i++) {
      reinterpret_cast<T*>(buffer + (i * sizeof(T)))->~T();
    }
  }


};

#define _storage_add(storage, item, T) \
    new(&storage.buffer[storage.stack_count * sizeof(T)]) item; \
    storage.stack_count++;

// synonym
#define _storage_push _storage_add



/*
// RECURSION PSEUDO CODE
func recursive_tree_sum(root: Node) {
    if(root == nullptr) {
        return 0;
    }
    
    return root.val + recursive_tree_sum(root.left) + recursive_tree_sum(root.right);
}
*/

int _recursive_sum_tree(const shared_ptr<TreeNode>& node) {

    if(node.get() == nullptr) {
        return 0;
    }

    /*
    // Less perplexing to translate if we are look at this like the following:
    // It is also the most efficient to look at the assembly output that the compiler generates for it below.
    int n = node.get()->val;
    int n1 = _recursive_sum_tree(node.get()->left);
    int n2 = _recursive_sum_tree(node.get()->right);
    return n + n1 + n2;
    */

    return node.get()->val + _recursive_sum_tree(node.get()->left) + _recursive_sum_tree(node.get()->right);

}

/*
// ASSEMBLY
_recursive_sum_tree(std::shared_ptr<TreeNode> const&):
        push    rbp
        push    rbx
        sub     rsp, 8
        mov     rbp, QWORD PTR [rdi]
        mov     eax, 0
        test    rbp, rbp
        je      .L15
        mov     ebx, DWORD PTR [rbp+0]
        lea     rdi, [rbp+8]
        call    _recursive_sum_tree(std::shared_ptr<TreeNode> const&)
        add     ebx, eax
        lea     rdi, [rbp+24]
        call    _recursive_sum_tree(std::shared_ptr<TreeNode> const&)
        add     eax, ebx
.L15:
        add     rsp, 8
        pop     rbx
        pop     rbp
        ret
*/

inline CallFrame make_call1(const shared_ptr<TreeNode>*, void*) __attribute__((always_inline));

inline CallFrame make_call1(const shared_ptr<TreeNode>* left, void* call1_origin) {
    CallFrame call1 = CallFrame();
    call1.args.node = left;
    call1.ret.origin = call1_origin;

    // Copy Elision
    return call1;

}

inline CallFrame make_call2(const shared_ptr<TreeNode>*, void*) __attribute__((always_inline));

inline CallFrame make_call2(const shared_ptr<TreeNode>* right, void* call2_origin) {
    CallFrame call2 = CallFrame();
    call2.args.node = right;
    call2.ret.origin = call2_origin;

    // Copy Elision
    return call2;

}

// TRANSLATION OF THE FUNCTION ABOVE
int _sum_tree(const shared_ptr<TreeNode>& _node) {

    // NOTE: CAN'T INLINE THIS FUNCTION DUE TO THE COMPUTED GOTO: YUCK!
    // __label__ call1_origin;
    // __label__ call2_origin;

    // void *jmps_origin[] = { &&call1_origin, &&call2_origin };

    // Can be dynamically allocated stack, which means we can call it as long as there is memory available
    StaticStorage<CallFrame, 20> stack;
    // static StaticStorage<CallFrame, 20> stack;


    CallFrame initial_call = CallFrame();
    initial_call.args.node = &_node;
    // PUSH
    _storage_push(stack, CallFrame(std::move(initial_call)), CallFrame);


    // CALL
    __sum_tree: {}

    // function args
    const shared_ptr<TreeNode>& node = *stack.last().args.node; // last: stack_c-1

    { // FUNCTION BODY
        if(node.get() == nullptr) {
            stack.last().ret.value = 0; // return 0;
            goto _ret;
        }

        // NOTE: DUE TO LABEL AND COMPUTED JUMPS, IT IS REQUIRED TO ENCLOSE THESE BLOCKS IN C++ DUE TO SKIPPING INITIALIZATION AND C++ RAII. THIS ALSO MAKES SENSE AT ASSEMBLY LEVEL
        // _print_in_order(node.get()->left);
        {
            // NOTE: Limit the scope of a by declaring it within a block statement that ends before the label.
            // REFERENCE and KUDOS TO: https://stackoverflow.com/a/70944477/14102601
            /*
            const shared_ptr<TreeNode>& node = *stack.last().args.node; // last: stack_c-1
            CallFrame call1 = CallFrame();
            call1.args.node = &node.get()->left;
            call1.ret.origin = &&call1_origin;
            // PUSH
            _storage_push(stack, CallFrame(std::move(call1)), CallFrame);
            // ~CallFrame()
            // CallFrame(CallFrame&&)
            */

            _storage_push(stack, CallFrame(make_call1(&node.get()->left, &&call1_origin)), CallFrame);

            // CALL
            goto __sum_tree;
        }
        // ORIGIN OF RET
        call1_origin: {}
        // ORIGIN_RET = stack[stack_c].ret.value;

        {
            // NOTE: NEEDS TO BE HERE SINCE AFTER COMPUTED GOTO TO CALL1_ORIGIN: THE NODE ABOVE IS LOST
            const shared_ptr<TreeNode>& node = *stack.last().args.node; // last: stack_c-1 // mov     rax, QWORD PTR [rbp-24]

            // C++: return node.get()->val + _recursive_sum_tree(node.get()->left)
            stack.last().ret.value = node.get()->val + stack.at(stack.stack_count).ret.value;
        }

        // _print_in_order(node.get()->right);
        {
            const shared_ptr<TreeNode>& node = *stack.last().args.node;

            /*
            CallFrame call2 = CallFrame();

            call2.args.node = &node.get()->right;
            call2.ret.origin = &&call2_origin;
            */

            // PUSH
            _storage_push(stack, CallFrame(make_call2(&node.get()->right, &&call2_origin)), CallFrame);

            // CALL
            goto __sum_tree;
        }
        // ORIGIN OF RET
        call2_origin: {}
        // ORIGIN_RET = stack[stack_c].ret.value;

        {
            // C++: return _recursive_sum_tree(node.get()->left) + _recursive_sum_tree(node.get()->right);
            stack.last().ret.value = stack.last().ret.value + stack.at(stack.stack_count).ret.value;
        }

    }

    _ret: {
        // Stack is empty
        /*
        if(stack_c == 0) {
            goto _done;
            // return;
        }
        */

        // pop rbx
        stack.pop();

        // NOTE: INITIAL PUSH ON LINE 137
        if(stack.stack_count == 0) {
            goto _done;
            // return;
        }

        // std::cout << "stack_c: " << stack_c << std::endl;

        // ret
        void* ret_origin = stack.at(stack.stack_count).ret.origin;
        goto *ret_origin;

    }


    _done: {}


    // ONLY FOR UNIT TESTS
    // assert(stack.stack_count == 0);


    return stack.at(stack.stack_count).ret.value;

}




#define _SIZE 10000

shared_ptr<TreeNode> _make_root() {
    shared_ptr<TreeNode> root = make_shared<TreeNode>(4);

    root.get()->left = make_shared<TreeNode>(2);
    root.get()->right = make_shared<TreeNode>(5);

    root.get()->left.get()->left = make_shared<TreeNode>(1);
    root.get()->left.get()->right = make_shared<TreeNode>(3);

    root.get()->right.get()->left = make_shared<TreeNode>(6);
    root.get()->right.get()->right = make_shared<TreeNode>(7);

    root.get()->right.get()->left->left = make_shared<TreeNode>(0);
    root.get()->right.get()->left->right = make_shared<TreeNode>(-1);

    root.get()->right.get()->right->left = make_shared<TreeNode>(8);
    root.get()->right.get()->right->right = make_shared<TreeNode>(9);

    root.get()->right.get()->right->right->left = make_shared<TreeNode>(77);
    root.get()->right.get()->right->right->right = make_shared<TreeNode>(88);


    root.get()->right.get()->left->left->left = make_shared<TreeNode>(99);
    root.get()->right.get()->left->left->right = make_shared<TreeNode>(111);


    return root;
}


int main1() {

    shared_ptr<TreeNode> root = _make_root();
    // 415


    int _result = _sum_tree(root);

    std::cout << "_result: " << _result << std::endl;

    assert(_result == _recursive_sum_tree(root));

    root = make_shared<TreeNode>(4);
    _result = _sum_tree(root);

    std::cout << "_result: " << _result << std::endl;

    assert(_result == _recursive_sum_tree(root));


    return 0;
}




void benchmark_01() {
    shared_ptr<TreeNode> root = _make_root();

    start_time;
    for(size_t i = 0; i < _SIZE; i++) {
        volatile int _s = _recursive_sum_tree(root);
    }

    end_time;


}


void benchmark_02() {
    shared_ptr<TreeNode> root = _make_root();

    start_time;
    for(size_t i = 0; i < _SIZE; i++) {
        volatile int _s = _sum_tree(root);
    }

    end_time;

}


int main() {

    /*
    shared_ptr<TreeNode> root = make_shared<TreeNode>(4);


    root.get()->left = make_shared<TreeNode>(2);
    root.get()->right = make_shared<TreeNode>(5);

    root.get()->left.get()->left = make_shared<TreeNode>(1);
    root.get()->left.get()->right = make_shared<TreeNode>(3);

    root.get()->right.get()->left = make_shared<TreeNode>(6);
    root.get()->right.get()->right = make_shared<TreeNode>(7);


    print_inorder(root);
    std::cout << std::endl;


    _print_order(root);
    std::cout << std::endl;
    */


    benchmark_01();

    benchmark_02();


    return main1();

}
